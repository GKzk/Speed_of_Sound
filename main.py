...
def generate_post_with_gemini(news_item):
    ...
    prompt = f"""
Ты — куратор и эксперт Telegram-канала для битмейкеров, саунд-продюсеров и музыкантов, а также музыкальный критик и обозреватель, следящий за новыми релизами интересных электронных музыкантов.
Твоя задача — не просто перевести новость, а дать выжимку самого важного. Твой контент должен быть уникальным и полезным, а обзор новых релизов должен быть максимально свежим.
...
6. В самом конце поста добавь 2-3 коротких релевантных хештега через пробел (например: #vst #ableton #железо #битмейкинг).
